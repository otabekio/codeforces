...
public class Solver
{
    ...
    void Solve()
    {
        ...
        for (int i = 0; i < n; i++) {
            int x = ri;
            if (a.ContainsKey(x)) a[x]++; else a[x] = 1;
        }  
        ...
        Console.WriteLine(a.Count == 0? "YES" : "NO");
    }

    bool go(int num, Dictionary<int, int> a) {
        if (num < 1) return false;

        if (a.ContainsKey(num)) {
            a[num]--;
            if (a[num] == 0) a.Remove(num);
            return true;
        } 

        if (num % 2 == 0) {
            return go(num/2, a) && go(num/2, a); 
        }
        
        return go(num/2, a) && go(num/2 + 1, a); 
    }
